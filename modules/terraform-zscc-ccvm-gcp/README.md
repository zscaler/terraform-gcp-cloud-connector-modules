...
**Support for deploying these images are deprecated as of April 15, 2026
...
| Name | Type |
|------|------|
| [google_compute_autoscaler.cc_asg](https://registry.terraform.io/providers/hashicorp/google/latest/docs/resources/compute_autoscaler) | resource |
...
| <a name="input_stateful_delete_rule"></a> [stateful\_delete\_rule](#input\_stateful\_delete\_rule) | A value that prescribes what should happen to the stateful disk when the VM instance is deleted. The available options are NEVER and ON\_PERMANENT\_INSTANCE\_DELETION. NEVER - detach the disk when the VM is deleted, but do not delete the disk. ON\_PERMANENT\_INSTANCE\_DELETION will delete the stateful disk when the VM is permanently deleted from the instance group. | `string` | `"ON_PERMANENT_INSTANCE_DELETION"` | no |
| <a name="input_tags"></a> [tags](#input\_tags) | (Optional) Tags to attach to the instance for purposes such as modifying routes applied to the instances | `list(string)` | `[]` | no |
...
| <a name="input_update_policy_max_surge_fixed"></a> [update\_policy\_max\_surge\_fixed](#input\_update\_policy\_max\_surge\_fixed) | The maximum number of instances that can be created above the specified targetSize during the update process. Conflicts with max\_surge\_percent. If neither is set, defaults to 1 | `number` | `0` | no |
...
| Name | Description |
|------|-------------|
| <a name="output_autoscaler_id"></a> [autoscaler\_id](#output\_autoscaler\_id) | GCP Autoscaler ID |
| <a name="output_autoscaler_self_link"></a> [autoscaler\_self\_link](#output\_autoscaler\_self\_link) | GCP Autoscaler Self Link |
