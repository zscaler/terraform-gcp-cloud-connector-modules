...
This example deployment script looks for a local zip file of the Cloud Run Function code in the root (e.g. cc_asg) function_zip/ directory with the name matching var.cloud_function_source_object_name ie: var.cloud_function_source_object_path. You have the option to upload that zip file to a new or existing storage bucket as a new object based on the variable upload_cloud_function_zip.
