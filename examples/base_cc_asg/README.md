# Zscaler "base_cc_asg" deployment type
...
Additionally: Creates 1 Cloud Connector compute instance template + between [1-3] zonal autoscaling policy enabled managed instance groups to deploy Cloud Connector appliances with a dedicated service account associated for accessing Secret Manager. This template also leverages the terraform-zscc-ilb-gcp module to create the necessary backend service, forwarding rule, health check, and firewall rules needed to front all cloud connector instances for highly available/resilient workload traffic forwarding. This example also deploys the `terraform-zscc-cloud-function-gcp` module, which includes two Cloud Functions for health monitoring and resource synchronization between GCP and Zscaler, along with Cloud Scheduler jobs to trigger them.

![base_cc_asg](https://github.com/zscaler/terraform-gcp-cloud-connector-modules/blob/topologies/docs/assets/example_topologies/base_cc_asg.svg)
...
This example deployment script looks for a local zip file of the Cloud Run Function code in the root (e.g. base_cc_asg) function_zip/ directory with the name matching var.cloud_function_source_object_name ie: var.cloud_function_source_object_path. You have the option to upload that zip file to a new or existing storage bucket as a new object based on the variable upload_cloud_function_zip.
...
| <a name="input_marketplace_image"></a> [marketplace\_image](#input\_marketplace\_image) | Available marketplace image name to deploy. Zscaler recommends always deploying new instances with the latest image | `string` | `"zs-cc-ga-2-08132026"` | no |
